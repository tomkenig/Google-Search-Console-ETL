...
# samples: https://github.com/googleapis/google-api-python-client/tree/master/samples
# pyodbc sql server doc https://github.com/mkleehammer/pyodbc/wiki/Getting-started
# GSC API Dimensions https://developers.google.com/webmaster-tools/search-console-api-original/v3/searchanalytics/query
info:
...
try:
    credentials = pickle.load(open("C:/Users/.../config/credentials.pickle", "rb"))
except (OSError, IOError) as e:
    flow = InstalledAppFlow.from_client_secrets_file("C:/Users/..json", scopes=OAUTH_SCOPE)
...
conn = pyodbc.connect('DRIVER={SQL Server};SERVER=YOUR_HOST;DATABASE=DATABASE_NAME;UID=DB_USER;PWD=DB_PASS')
...
for date in out_date_range(start_date, date):
    for site in get_verified_siteurl_list():
        cursor.execute("delete from google.gsc.tbGscByWeb where gscSiteUrl=? and gscDate=?", site, date)
        cursor.execute("insert into google.gsc.tbGscByWeb(gscData, gscSiteUrl, gscDate) values (?, ?, ?)", json.dumps(out_gsc_data(site, str(date)[0:10])), site, date)
        conn.commit()
        print(site)
    time.sleep(10)
